...
# like Debian's own, instead of sitting as manual-only entries in GNOME
# Software -- see lib/add-vendor-origins-unattended-upgrades.sh for the
# exact Origin/Codename values (Signal's Release file really does say
# "Origin: . xenial").
...
# 53-virtualbox.sh for VirtualBox, Cyberbeest Package Manager for Chrome --
# all via lib/cyberbeest-pkg-helper.sh setup-repo) -- writing them here
# first is harmless, so this doesn't need to run after any of those despite
# the higher numbers.
