...
# lib/build_messenger_catalog.py for how metadata/icons are pulled from each
# package's own upstream files, and lib/install-messenger-catalog.sh for the
...
# stack). Signal/Element's vendor apt repos are set up automatically by
# lib/build_messenger_catalog.py itself (via lib/cyberbeest-pkg-helper.sh
# setup-repo), so no separate repo-registration step is needed here.
...
bash "$DIR/lib/install-messenger-catalog.sh"
echo "=== done ==="
